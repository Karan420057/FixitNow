...
const LocationToast = ({ type, onClose }) => {
  ...
  return (
    <div className={`flex items-center gap-3 p-3.5 rounded-xl border animate-slide-up ${cls}`}>
      {icon}
...
export const RegisterPage = () => {
  const [form, setForm] = useState({
    ...
    role: 'customer', location: '', category: '', serviceArea: ''
  });
  ...
  useEffect(() => {
    ...
    setForm({ name: '', email: '', password: '', confirmPassword: '', role: 'customer', location: '', category: '', serviceArea: '' });
    setShowPass(false);
  ...
  const handleFileChange = (e) => {
    ...
    if (!ACCEPTED_MIME.includes(file.type)) {
      setErrors(prev => ({ ...prev, idFile: 'Only JPG, PNG, or PDF files are accepted.' }));
      return;
    }
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      setErrors(prev => ({ ...prev, idFile: `File size must be under ${MAX_FILE_SIZE_MB}MB.` }));
      return;
    }
    setIdFile(file);
  ...
  const validate = () => {
    ...
    if (form.role === 'provider') {
      ...
      if (selectedSlots.length === 0)            e.timeSlots       = 'Please select at least one time slot';
    ...
    <div className="min-h-screen animated-gradient flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-lg relative z-10 animate-slide-up py-8">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-dark-400 text-sm mt-1">Create your account to get started</p>
        </div>
        ...
        <div className="bg-glass rounded-2xl p-6 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {['customer', 'provider'].map(role => (
              ...
              >
                <div className="text-2xl mb-1">{role === 'customer' ? '👤' : '🛠️'}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 sm:col-span-1">
                ...
                <input type="text" placeholder="John Doe" value={form.name} onChange={e => f('name', e.target.value)} className="input-field" />
                {errors.name && <p className="text-red-400 text-xs mt-1">{errors.name}</p>}
              </div>
              <div className="col-span-2 sm:col-span-1">
                <label className="text-sm font-medium text-dark-300 mb-1.5 block">Email</label>
                <input type="email" placeholder="you@example.com" value={form.email} onChange={e => f('email', e.target.value)} className="input-field" />
            ...
            <div>
              ...
              <input type="text" placeholder="e.g. Koramangala, Bengaluru" value={form.location} onChange={e => f('location', e.target.value)} className="input-field" />
            ...
            <div className="p-4 bg-dark-900/50 rounded-xl border border-dark-700">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs font-mono uppercase tracking-wider text-dark-400">Set Precise Location</p>
              </div>
              <p className="text-xs text-dark-500 mb-3">
                Capture your exact coordinates so customers can find you accurately
              </p>
            ...
            {form.role === 'provider' && (
              <>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-dark-900/50 rounded-xl border border-dark-700">
                  <p className="col-span-2 text-xs font-mono uppercase tracking-wider text-dark-400">Provider Details</p>
                  <div>
                    ...
                    <select value={form.category} onChange={e => f('category', e.target.value)} className="input-field">
                      ...
                      {['Electrical', 'Plumbing', 'Carpentry', 'AC Repair', 'Painting', 'Cleaning', 'Appliance', 'Security'].map(c => (
                  ...
                  <div>
                    <label className="text-sm font-medium text-dark-300 mb-1.5 block">Service Area (km)</label>
                    <select value={form.serviceArea} onChange={e => f('serviceArea', e.target.value)} className="input-field">
                ...
                <div className="p-4 bg-dark-900/50 rounded-xl border border-dark-700">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-xs font-mono uppercase tracking-wider text-dark-400">Available Time Slots</p>
                    <span className="text-red-400 text-xs">*required</span>
                  </div>
                  <p className="text-xs text-dark-500 mb-3">Select all time slots when you are available to work</p>
                  <div className="flex flex-wrap gap-2">
                    {TIME_SLOTS.map(slot => {
                      ...
                      return (
                        ...
                        >
                          {isSelected && <CheckCircle className="w-3 h-3 text-brand-400" />}
                          {slot.label}
                        </button>
                      );
                    })}
                  ...
                  {errors.timeSlots && <p className="text-red-400 text-xs mt-2">{errors.timeSlots}</p>}
                </div>
                ...
                <div className="p-4 bg-dark-900/50 rounded-xl border border-dark-700">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-xs font-mono uppercase tracking-wider text-dark-400">ID Proof Verification</p>
                    <span className="text-red-400 text-xs">*mandatory</span>
                  </div>
                  <p className="text-xs text-dark-500 mb-3">
                    Upload a valid government-issued ID. Accepted: Aadhaar Card, PAN Card, Driving License
                  </p>
                  <div className="mb-3">
                    <label className="text-sm font-medium text-dark-300 mb-1.5 block">Document Type</label>
                    <select
                      value={idDocType}
                      onChange={e => { setIdDocType(e.target.value); setErrors(prev => ({ ...prev, idDocType: '' })); }}
                      className="input-field"
                    >
                      <option value="">Select document type</option>
                      {ACCEPTED_DOC_TYPES.map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                    {errors.idDocType && <p className="text-red-400 text-xs mt-1">{errors.idDocType}</p>}
                  </div>
                    ...
                    {idFile ? (
                      <>
                        ...
                        <div className="text-center">
                          <p className="text-green-400 text-sm font-medium">{idFile.name}</p>
                          <p className="text-dark-400 text-xs mt-0.5">
                            {(idFile.size / 1024).toFixed(0)} KB • Click to replace
                          </p>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="text-center">
                          <p className="text-dark-300 text-sm font-medium">Click to upload ID proof</p>
                          <p className="text-dark-500 text-xs mt-0.5">JPG, PNG or PDF • Max {MAX_FILE_SIZE_MB}MB</p>
                        </div>
                      </>
                    )}
                  </label>
                  {errors.idFile && <p className="text-red-400 text-xs mt-2">{errors.idFile}</p>}
                </div>
              </>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-dark-400 hover:text-dark-200 transition-colors">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="input-field pr-10"
                  />
                  <button type="button" onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-dark-400 hover:text-dark-200 transition-colors">
            ...
            <div className="flex items-start gap-2 text-sm">
              <input type="checkbox" required className="accent-brand-500 mt-0.5 flex-shrink-0" />
              <p className="text-dark-400">
                I agree to the{' '}
                <button type="button" className="text-brand-400 hover:underline">Terms of Service</button>
                {' '}and{' '}
                <button type="button" className="text-brand-400 hover:underline">Privacy Policy</button>
              </p>
            </div>
            ...
            {form.role === 'provider' && (
              <div className="flex items-start gap-2.5 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                ...
                <p className="text-yellow-300 text-xs leading-relaxed">
                  Provider accounts require admin approval. After registration, your profile will be reviewed before you can start accepting bookings.
                </p>
              </div>
            )}
