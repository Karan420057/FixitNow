...
export const ServiceDetailPage = () => {
  ...
  return (
    <div className="animate-fade-in">
      {/* Back button */}
      <Link to="/customer/services" className="inline-flex items-center gap-2 text-dark-400 hover:text-white mb-6 transition-colors group">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between flex-wrap gap-2">
                  <div>
                    <h1 className="font-display font-bold text-2xl text-white">{service.category}</h1>
                    <p className="text-dark-400 mt-0.5">{service.subcategory}</p>
                  </div>
                  {service.verified && (
                ...
                <div className="flex items-center gap-4 mt-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-dark-400 text-sm">({service.reviews} reviews)</span>
                  </div>
                  <span className="flex items-center gap-1 text-dark-400 text-sm">
                    <MapPin className="w-4 h-4" /> {service.distance}
                  </span>
                  <span className="flex items-center gap-1 text-dark-400 text-sm">
                    <CheckCircle className="w-4 h-4 text-green-400" /> {service.completedJobs} jobs done
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Provider Info */}
          <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
            <h3 className="font-display font-semibold text-lg text-white mb-4">Service Provider</h3>
            <div className="flex items-center justify-between">
              ...
              <Link to={`/customer/chat`} className="btn-secondary flex items-center gap-2 py-2 text-sm">
            ...
            <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-dark-700">
              {[
                { label: 'Response Time', value: '< 1 hr', icon: <Clock className="w-4 h-4 text-blue-400" /> },
                { label: 'Completed Jobs', value: service.completedJobs, icon: <CheckCircle className="w-4 h-4 text-green-400" /> },
                { label: 'Rating', value: `${service.rating}/5`, icon: <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" /> },
              ].map(item => (
                <div key={item.label} className="text-center p-3 bg-dark-900/50 rounded-xl">
          ...
          {/* Reviews */}
          <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
            <h3 className="font-display font-semibold text-lg text-white mb-4">Customer Reviews</h3>
            <div className="space-y-4">
              {REVIEWS.map(rev => (
                <div key={rev.id} className="pb-4 border-b border-dark-700 last:border-0 last:pb-0">
        ...
        <div className="lg:col-span-1">
          <div className="bg-dark-800 border border-dark-700 rounded-2xl p-5 sticky top-20">
            <div className="mb-5">
              ...
              <p className="font-display font-bold text-3xl text-brand-400">₹{service.price}</p>
              <p className="text-dark-500 text-xs mt-0.5">+ taxes as applicable</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {/* Notes */}
              <div>
                <label className="text-sm font-medium text-dark-300 mb-2 block">Additional Notes</label>
                <textarea
                  rows={3}
              ...
              <p className="text-xs text-dark-500 text-center flex items-center justify-center gap-1">
                <Shield className="w-3.5 h-3.5" /> Free cancellation up to 24 hours before
              </p>
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={bookingModal} onClose={() => !confirmed && setBookingModal(false)} title="Confirm Your Booking" size="md">
        {confirmed ? (
          <div className="text-center py-6">
            <div className="w-16 h-16 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle className="w-8 h-8 text-green-400" />
            </div>
            <h3 className="font-display font-bold text-xl text-white mb-2">Booking Confirmed!</h3>
            <p className="text-dark-400 text-sm">Redirecting to your bookings...</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmBooking} className="btn-primary flex-1">Pay & Confirm</button>
            </div>
          </div>
        )}
      </Modal>
